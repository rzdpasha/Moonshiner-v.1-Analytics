...
####################### Демо-модели для показа наружу (с обезличенными клиентами)####################
class DemoBuyer(models.Model):
    client = models.CharField(max_length=100, verbose_name="Анонимный покупатель")
    original_id = models.PositiveIntegerField(unique=True, db_index=True)
    ...
    last_purchase_date = models.DateTimeField(
        ...
        verbose_name="Последний раз был"
    )

    def __str__(self):
        return f"{self.client} ({self.abc_category or '?'})"


class DemoIncome(models.Model):
    date = models.DateTimeField("Дата продажи", blank=True, null=True)
    client = models.ForeignKey(DemoBuyer, on_delete=models.CASCADE)

    category = models.ForeignKey(
        "homebrew.Category", on_delete=models.PROTECT, null=True
    )
    variant = models.ForeignKey("homebrew.ProductVariant", on_delete=models.PROTECT)
    ...
    order_group_id = models.IntegerField(
        null=True, blank=True, verbose_name="ID группы заказа"
    )
